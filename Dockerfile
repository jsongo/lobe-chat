...
RUN <<'EOF'
set -e
# Keep only package management files, delete everything else
find . -mindepth 1 -maxdepth 1 ! -name "package.json" ! -name "pnpm-workspace.yaml" ! -name ".npmrc" ! -name "packages" ! -name "apps" ! -name "node_modules" -exec rm -rf {} + || true
# In packages and apps, keep only package.json files
find packages -mindepth 2 -type f ! -name "package.json" -delete || true
find packages -mindepth 2 -type d -empty -delete || true
find apps -mindepth 2 -type f ! -name "package.json" -delete 2>/dev/null || true
find apps -mindepth 2 -type d -empty -delete 2>/dev/null || true
EOF
...
ARG FEATURE_FLAGS
ARG NODE_MAX_OLD_SPACE_SIZE="10240"
...
ENV NEXT_PUBLIC_ENABLE_BETTER_AUTH="${NEXT_PUBLIC_ENABLE_BETTER_AUTH:-0}" \
    ...
    # Better Auth secret for build - prevents default secret warning (must be >= 32 chars)
    AUTH_SECRET="build-time-secret-for-nextjs-standalone-mode-complete-32chars"
